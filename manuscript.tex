...
\affil[19]{, \orgname{Consortium member}}
...
whose sizes can encompass up to tens of thousands of features and
samples in downstream statistical analyses \citep{kuhn2026}.
...
\citep{moreno-indias2021, jeganathan2021, willis2019, bastiaanssen2023_1, bastiaanssen2023_2},
...
identify the suitable methods and build interoperable workflows
\citep{wen2023, shetty2019}. Accordingly, the field can benefit from
...
packages \citep{drnevich2025}.

Here, we introduce a stable and growing data science ecosystem that
supports the integration and analysis of multi-modal datasets, with a
particular focus on microbiome research. It provides harmonized data
import and representation, and fast, robust methods for transforming
abundance tables into biological insights. These methods and workflows
are also applicable to host-level omics, such as tabular
transcriptomics, proteomics, and metabolomics data. This generality
supports a broad applicability, interoperability, and translation of
methodology across application domains. To promote evidence-based best
practices, we introduce the online book \emph{Orchestrating Microbiome
Analysis with Bioconductor} (OMA), as a supporting resource which
represents a compilation of established approaches and emerging trends
in microbiome data science using the R/Bioconductor ecosystem. The
underlying methods ecosystem has been extensively tested and distributed
through mature R/Bioconductor packages
(\url{https://bioconductor.org/packages}), facilitating the continuing
...
networks), and supporting side information (\emph{e.g.}, sample
metadata). This side information can include sample attributes such as
treatment group, collection site, or time point. The ability to
integrate heterogeneous data elements is essential in the study of
biological systems. However, the individual components of a system are
often studied in isolation due to limitations in data availability, or
because the integration of data types can be challenging. This growing
complexity in incorporating heterogeneous data types introduces
additional overhead, as researchers must track samples and features
across multiple tables and manage an increasing number of data elements
and non-trivial links between them. Moreover, input data formats vary
across methods, leading to technical complications and time lost on data
wrangling. Bioconductor addresses these challenges through standardized
data structures that are extended by an ecosystem of quality-controlled,
interoperable methods. Adopting the framework will thus allow the
analyst to avoid unnecessary data wrangling and invest more time in core
analytical tasks.
...
workflows across the Bioconductor ecosystem. Hereafter, we use TreeSE to
refer specifically to the TreeSummarizedExperiment class, and (Tree)SE
to refer collectively to SummarizedExperiment and
TreeSummarizedExperiment and, where relevant, their shared ecosystem of
methods.
...
\includegraphics[width=0.5\textwidth,height=\textheight]{figures/data_containers.png}

}

\caption{\label{fig-datacontainers}\textbf{Integrative Bioconductor
containers for microbiome data.} \textbf{(a) SummarizedExperiment (SE)}
\cite{huber2015} is a generic container for single-omics experiments. It
accommodates \emph{assays} that describe the feature abundances in each
sample (\emph{e.g.}, taxonomic units, resistance genes, or predicted
functions), and tabular side information on each sample (\emph{colData})
and feature (\emph{rowData}), \emph{e.g.}, the taxonomic classification.
\textbf{(b) TreeSummarizedExperiment (TreeSE)} \cite{huang2021} supports
hierarchical, multi-table microbiome analysis. In addition to the SE
elements, TreeSE incorporates tree information on feature hierarchies,
such as phylogenetic relations between the taxonomic units
(\emph{rowTree}) or host species (\emph{colTree}), and results of
dimensionality reduction (\emph{reducedDims}). TreeSE can thus link
multiple abundance tables and their transformed versions (\emph{e.g.},
statistical transformation, taxonomic agglomeration, or dimensionality
...
sequences and experiment metadata are available. \textbf{(c)
MultiAssayExperiment (MAE)} \cite{ramos2017} facilitates the integration
of interlinked datasets representing different omics modalities linked
by potentially non-trivial sample mappings. Figure adapted from
\cite{ramos2017}.}

\end{figure}%

\textbf{Multi-assay data structures.} Contemporary microbiome research
frequently involves data across multiple omics modalities for enhanced
functional and mechanistic insights. This brings the need to find an
appropriate representation for each data type and map the links between
them. Whereas the (Tree)SE data container is broadly applicable to
...
irreproducibility in ad hoc multi-table analyses. A flexible mapping of
samples across several data objects is a key for efficient data
integration, method sharing, and interoperability.
...
retrieval to TreeSE format instructed in the manuscript source code.
\(^c\)Deposited in the Zenodo mia community. \(^d\)HoloFood microbiome
datasets are deposited in the MGnify database. HMP, Human Microbiome
Project.
...
adopted tools with a primary focus on 16S amplicon data analysis. TreeSE
accommodates all core components of phyloseq objects, including
abundance tables, taxonomic annotations, phylogenetic trees, reference
sequences, and sample metadata. The advantages of newer TreeSE container
\citep{huber2015} include many additional data elements, a better
integration with the overall Bioconductor ecosystem, and enhanced
...
\citep{bolyen2019} is a Python-based microbiome multi-omics data science
platform. It primarily targets amplicon and metagenomic analyses, and
incorporates methods from the scikit-bio Python library
...
community-driven analysis and visualization platform for microbial omics
with a distinct focus on genome-resolved metagenomics and detailed
sequence-level analyses. These frameworks emphasize standardized,
modular approaches tailored to microbiome analysis and provide validated
and user-friendly solutions, supporting efficient end-to-end workflows
for microbiome research. Bioconductor emphasizes broad interoperability
and shared statistical programming infrastructure with other areas of
omics research, and primarily serves the R community. To facilitate
migration from alternative frameworks, the OMA book (Chapters 4 and 5,
Appendix A) provides conversion tables and utility functions to convert
between TreeSE and many external data formats.
...
time and memory consumption with (Tree)SE, phyloseq, speedyseq, and
QIIME 2. Overall, (Tree)SE scaled most efficiently in execution time and
...
feature set size of 10,000 the break points drop down to 10,000 samples
with agglomeration and Faith diversity calculation, and to 1,000 samples
with the PhILR transformation and melting operation. At smaller sample
sizes phyloseq or its accelerated extension speedyseq are occasionally
faster but these differences are moderate (few seconds on a standard
laptop). The standard QIIME 2 implementation is generally slower on
smaller sample sizes but scales well with increasing sample sizes. It
should be noted that these benchmarks focus on methods that are
currently accessible via R/Bioconductor, and thus benefit from the
broader interoperability with this framework. The users could benefit
from the incorporation of the most recent developments in accelerated
methods, such as DartUniFrac \citep{zhao2026}, which provides massive
scalability enhancements for Unifrac-based phylogenetic distance
calculation. Optimizing time and memory consumption represents a key
area for further development in microbiome data science.
...
contemporary microbiome studies. Common assay types include
amplicon-based marker gene studies (\emph{e.g.}, 16S rRNA) and
metagenomic sequencing, processed with methods such as MetaPhlAn
\citep{blanco2023} or Greengenes2 \citep{mcdonald2024}. Alternative
...
\citep{abakumov2024}. Tabular abundance data can also arise from
...
exploration and visualization (Figure \ref{fig:workflow}).
...
unequal sampling depth is data rarefaction, \emph{i.e.} repeatedly
subsampling data to equal read depth and averaging the computed metrics
to mitigate information loss \citep{schloss2024b} that could arise from
subsampling (\emph{rarefying}) data to equal read depth without such
iteration \citep{mcmurdie2014, schloss2024, willis2019b}. Our framework
supports these and a variety of other transformations commonly applied
to microbiome data.
...
analyses between modalities. A comprehensive set of recommendations and
resources for microbiome analysis are maintained in the online book
(Chapter 2).
...
Community diversity is a common measure in microbial ecology and
epidemiology \citep{bastiaanssen2023_1}
(Figure~\ref{fig-downstream_analyses}). Alpha diversity metrics
generally quantify the observed or estimated number of distinct taxa
(\emph{richness}) and their distribution (\emph{evenness}). Many
diversity indices (\emph{e.g.}, Shannon index), combine these two
aspects. Phylogenetic diversity metrics account for the breadth of
phylogenetic relatedness among community members, but are more
computationally demanding than abundance-based metrics. A recent
implementation, the Stacked Faith's Phylogenetic Diversity, improves
computational efficiency by 1--2 orders of magnitude
...
\citep{schloss2024b} through the \emph{mia} package. Other global
...
\textbf{Community similarity.} This is commonly evaluated with beta
diversity measures, which can be used for quantitative comparisons of
multivariate community composition. Our framework supports common
dissimilarity metrics in microbiome research, including Bray-Curtis,
Jaccard, Aitchison \citep{bastiaanssen2023_1}, and the accelerated
phylogeny-aware (striped) UniFrac \citep{mcdonald2018}. Community
dissimilarity is commonly visualized with unsupervised ordination that
projects data onto a lower-dimensional space. Among unsupervised
techniques, the most commonly used ones include Principal Component
...
\citep{frioux2023, lindgren2026}. These and other multivariate methods
are provided by mia, miaViz, and additional single-cell packages
(scater, scuttle \citep{amezquita2020}). Many omics modalities can
benefit from these and other standardized methods.
...
abundance data, and focus on comparing feature prevalence \emph{i.e.}
presence/absence profiles. Recent implementations include MaAsLin3
...
data, univariate to multivariate and multi-omic monitoring, and real,
pseudo, or simulated time series
\citep{velten2022, sherwani2025, fukuyama2017}. This heterogeneity
brings additional challenges for standardizing the methods. Moreover, an
increasing number of studies incorporate spatial dimension in microbiome
analysis \citep{ruuskanen2021}. While spatio-temporal analyses require
...
feature exhibits multiple abundance states). Methods such as
compositional tensor factorization (CTF) \citep{martino2021} and
temporal tensor decomposition (TEMPTED) \citep{shi2024} were recently
proposed in a single-omic setting to model community-level shifts in
microbiome composition while accounting for the subject-specific
variation, and to characterize community dynamics by aggregating
information across short time series \citep{ross2026}. Another type of
temporal analysis includes the time-to-event, or survival models, which
are encountered regularly in epidemiological and population cohort
studies; for microbiome data, these analyses require specialized
approaches, as proposed in previous studies \citep{salosensaari2021}. We
...
miaSim package \citep{gao2023}. Our framework provides a robust basis
for developing extended methods for longitudinal multi-omics, which is
currently a remarkable methodological gap and an active area of
investigation \citep{sherwani2025}. We converted a large-scale time
series of 16,234 gut microbiome profiles from 585 wild baboons over 14
years \citep{grieneisen2021} into the TreeSE format to support training
and methods development for longitudinal microbiome data.

\textbf{Multi-modal integration.} A key advantage of this framework is
that it builds on the globally adopted infrastructure supporting the
integration of microbial abundance data with other omics modalities,
such as transcriptomes, proteomes, or metabolomes
\citep{hintikka2021, mangnier2025}, resistomes
\citep{salehi2025, lee2023}, host genomes \citep{qin2022}, or
environmental exposomes (Figure~\ref{fig-downstream_analyses}). By
leveraging standardized containers (Figure \ref{fig-datacontainers}),
...
between microbial features, and methods such as anansi
\citep{bastiaanssen2023} can additionally incorporate known functional
relations into association analyses. Unsupervised latent variable
methods such as multi-omics factor analysis (MOFA+)
\citep{argelaguet2020} and joint robust PCA (Joint-RPCA)
...
dependencies between omics layers to improve prediction performance.
These and other methods natively support Bioconductor's multi-assay
framework. Additional tools are provided by multi-omic toolkits such as
mixOmics \citep{rohart2017}, including the discriminant analysis method
DIABLO \citep{singh2019} and other integration techniques. These tools
do not yet support the same data containers but can be used seamlessly
by using converters or a straightforward extraction of the abundance
tables, sample metadata, or other relevant data elements. The unified
...
or microbe-set enrichment analyses \citep{kou2020, pham2025}. These and
other similar tools are complementary and they operate primarily on a
single feature matrix, lacking the multi-omic feature selection offered
by dedicated integration methods discussed in the previous section.
Interoperable containers facilitate the translation of existing tools to
adjacent or emerging disciplines.
...
enhanced by additional workflow management tools such as targets in R
\citep{landau2021} or Nextflow for cross-language operations
\citep{di2017}.
...
\caption{\label{fig-downstream_analyses}\textbf{Microbiome analysis
workflows in the Bioconductor ecosystem.} The figure demonstrates a
range of single-omic and multi-omic analytical approaches available for
microbiome research. Using a publicly available multi-omics mouse
dataset (taxonomic and metabolomic measurements) \citep{hintikka2021},
we illustrate: \textbf{(a) Taxonomic visualization} with phylogenetic
trees and abundance heatmaps; \textbf{(b) Community indicators} based on
Faith's phylogenetic diversity \citep{armstrong2021}; \textbf{(c)
Community similarity} using dbRDA based on UniFrac dissimilarities
\citep{lozupone2005}; \textbf{(d) Association analysis} with
differential prevalence testing \citep{nickols2026}; \textbf{(e)
Joint-RPCA biplot} \citep{vargas2026} and \textbf{(f) Supervised feature
ranking with IntegratedLearner} \citep{mallick2024}, illustrating
multi-omic integration of microbiota and metabolomics data. The former
uses unsupervised dimensionality reduction to identify shared variation
across data layers, whereas the latter uses supervised machine learning
to rank features by their strength of association with experimental
conditions or phenotypes.}
...
complemented with visual exploration and verification. Expressive code
and meaningful comments support reproducibility and verification. Such
practices can greatly improve the transparency and robustness of the
analyses and interpretability of the results. OMA facilitates
...
transcriptomics (OSTA book \citep{crowell2025}). These resources use
standard data containers to support statistical programming on
interlinked multi-table datasets.

\textbf{Interactive applications.} Graphical user interfaces provide
accessible entry points. The iSEEtree and miaDash packages facilitate
the interactive analysis and visualization of microbiome data
\citep{benedetti2025}. The former extends iSEE interactive graphics to
hierarchical data by adding support for the TreeSE class, whereas the
latter enhances iSEEtree with an extensive, mia-powered dashboard to
perform microbiome data analysis tasks, without requiring any
programming skills. For reproducibility, their interfaces provide
automatic generation of source code, replicable workflows, and analysis
templates for non-technical users. In parallel, the tidy R paradigm,
...
Bioconductor support site, Zulip, GitHub platform, and an email list.
The development is influenced by feedback from the user community
through these channels, regular meetings, and training workshops.
...
\citep{joos2024, bastiaanssen2023_1}. Technological advances in
...
techniques to link microbiome data with other omics along varying
spatial and temporal dimensions. Future extensions could thus provide
guidance for longitudinal and other repeated-measures microbiome
studies, where temporal dependency and intra-subject correlation
introduce additional analytical complexity. Despite the recent progress
\citep{martino2021, shi2024}, the lack of integrative multi-omic methods
for longitudinal analysis remains a notable gap in multi-omic research
\citep{sherwani2025}. Moreover, there is a need for integrated
multi-omics workflows that can combine microbiome data with other omics
to support interpretable hypotheses and causal analyses.

In concert with parallel developments in other omics, our framework
specifically addresses the need to jointly analyze interlinked abundance
tables while supporting hierarchical and tabular side information.
Future developments may necessitate the adoption of extended
hierarchical data structures such as Bioconductor's GraphExperiment. The
field could also benefit from improved support for the FAIR data
principles \citep{wilkinson2016}, for instance, by adding built-in
refinement, enrichment, and validation methods for community standards
\citep{field2008, mirzayi2021, kelliher2025}. Our framework provides the
structured input necessary for probabilistic inference and scalable
computation as the recent developments increasingly rely on advanced
statistical and ML techniques such as Bayesian modeling and Gaussian
...
evolves, the developer community will continue to integrate new tools
...
v4.6 and Bioconductor v3.24. Analytical scripts to reproduce this work
were deposited in the Zenodo mia community:
\url{https://doi.org/10.5281/zenodo.22691743}
...
S.S is employed by Danone Research and Innovation, and the employer had
no role in designing or executing the study. The other authors declare
